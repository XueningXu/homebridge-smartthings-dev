...
module.exports = {
  ...
  DHT2Capabilities_local: {
    ...
    'c2c-contact':  [['st.contactSensor'], ['st.contactSensor', 'st.temperatureMeasurement']], // st.battery is omitted
    'c2c-switch': [['st.switch']],
    // 'c2c-switch-power':	[['st.switch', 'st.powerMeter']],
    'c2c-leak': [['st.waterSensor'], ['st.waterSensor', 'st.temperatureMeasurement'], ['st.waterSensor', 'st.temperatureMeasurement', 'st.relativeHumidityMeasurement']],
    ...
    'c2c-siren-2': [['st.alarm']],
    'c2c-lock-2': [['st.lock']],
    'c2c-smoke-2': [['st.smokeDetector']],
  },
  ...
  DHT2Capabilities_ST: {
    ...
    // 'd09fe0f5-9717-47ee-9cd0-51dd59098d52': ['st.securitySystem', 'st.bypassable'],
    'd09fe0f5-9717-47ee-9cd0-51dd59098d52': ['st.securitySystem'],
    'c2c-siren-2': ['st.alarm'],
    'c2c-lock-2': ['st.lock', 'st.battery'],
    'c2c-smoke-2': ['st.smokeDetector'],
  },
  ...
  Capabilities2Attributes: {
    ...
    'st.bypassable': ['bypassStatus'],
    'st.alarm': ['alarm'],
    'st.lock': ['lock'],
    'st.smokeDetector': ['smoke'],
  },
  ...
  sanitizeValue: function(output, source) { // obj is state or event
    ...
    switch(output.attribute) {
      ...
      case 'alarm':
        switch(source.value) {
          case 0: 
            output.value = "both";     // hap --> st: stay_arm --> both
            break;
          case 1:
            output.value = "siren";   // hap --> st: away_arm --> siren
            break;
          case 3:
            output.value = "off";     // hap --> st: disarm --> off
            break;
        } 
        break;
      case 'lock':
        output.value = (source.value == 1) ? 'locked' : 'unlocked';
        break;
      case 'smoke':
        output.value = (source.value == 1) ? 'detected' : 'clear';
        break;
      case 'water':
        output.value = (source.value == 1) ? 'wet' : 'dry';
        break;
      case 'contact':
        output.value = (source.value == 1) ? 'open' : 'closed';
        break;
      case 'motion':
        output.value = (source.value == 1) ? 'active' : 'inactive';
        break;
      case 'presence':
  ...
  stCmd2HapValue: function(command) {
    ...
    debug('ST command method: ', method);
    debug('ST command capability: ', capability);
    ...
    switch(method) {
      ...
      case 'off':
        if(capability === 'st.switch') {
          value = 0;
        } else if (capability === 'st.alarm') {
          value = 3;
        }
        break;
      case 'armStay':
        if(capability === 'st.securitySystem') {
          value = 0;
        }
        break;
      case 'armAway':
