...
db.getPToken(platform).then((ptoken) => {
  ...
  .commandHandler(async (accessToken, response, device) => {
    var requestedDevice = devices[0];
    db.getDevice(requestedDevice.externalDeviceId).then(localdevice => {
      let promises = [];
      for(let cmd of requestedDevice.commands) {
        // TODO: send command to homebridge accessory. 
        promises.push();
      }
    ...
    .catch(err => {
      console.err("command handler error:", err);
      response.addDevice(requestedDevice.externalDeviceId);
      return response;
    })
  })
  .callbackAccessHandler(async (accessToken, clientId, callbackAuthentication, callbackUrls) => {
  ...
  .integrationDeleteHandler(accessToken => {
...
function formatDevice4ST(device_id, device_type, event) {
  var attributes = [];
  attributes.push(device_type);
  const dht = api.findDeviceHandlerTypeFromAttributes(attributes);
  
  if(dht) {
    var newdevice = new DiscoveryDevice(device_id, `${event.aid}:${event.iid}`, dht);
...
// TODO
function pushDeviceToTargetPlatform(device) {
  const device_ST = formatDevice4ST(device);
}
