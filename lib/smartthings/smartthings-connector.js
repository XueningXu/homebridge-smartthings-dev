...
db.getPToken(platform).then((ptoken) => {
  ...
  .discoveryHandler(async (accessToken, response) => {
    await db.getDevices().then((devices) => {
      var dev;
      for (dev of devices) {
        // addDevice('Wemo Mini', 'Wemo Mini_2:10', 'Outlet'--> defined attribute), TODO: change device_type as an array
        var device = response.addDevice(dev.device_id, `${dev.device_id}_${dev.aid}:${dev.iid}`, api.findDeviceHandlerTypeFromAttributes([dev.device_type]));
    ...
    db.getDevice(requestedDevice.externalDeviceId).then(device => {
      ...
      for(let cmd of requestedDevice.commands) {
        // send smartthings command to homebridge accessory. 
	      var cmdValue = stCmd2HapValue(cmd); 
