...
var api = require('./smartthings/smartthings-api.js')
...
function registerEvents(message) {
  ...
  for (var key in message) {
    ...
    api.pushDeviceToTargetPlatform(device, friendlyName, deviceType);
    ...
    switch (event.status) {
      ...
      case 0:
        if (event.value !== undefined) {
          ...
          // write event to file
          axios.post('http://remains.ddns.net:20000/event', {
          ...
          .then( (response) => {
            console.log("success write [%s : %s] to file with code %s", event.deviceID, event.value, response.status);
          })
          .catch( (error) => {
            console.log(error);
          });
          ...
          api.pushEventToTargetPlatform(device.friendlyName, device.deviceType, event);
