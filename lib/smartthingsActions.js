...
var pushDeviceToTargetPlatform = require('./smartthings/smartthings-connector.js').pushDeviceToTargetPlatform;
var pushEventToTargetPlatform = require('./smartthings/smartthings-connector.js').pushEventToTargetPlatform;
...
module.exports = {
  ...
  smartthings2homebridge: smartthings2homebridge
};
...
function registerEvents(message) {
  ...
  //  {value: value, endpointID: 'XXXXXX', deviceType: 'Outlet', friendlyName: 'Wemo Mini'}
  // ]
  ...
  for (var key in message) {
    ...
    var cur_value = message[key].value;
    ...
    db.insertState(device, cur_value, deviceType, friendlyName);
    
    // push device to smartthings cloud
    pushDeviceToTargetPlatform(device, friendlyName, deviceType);
    ...
    switch (event.status) {
      ...
      case 0:
        if (event.value !== undefined) {
          ...
          db.updateState(device.friendlyName, device.deviceType, event);
          
          // send event to smartthings
          pushEventToTargetPlatform(device.friendlyName, device.deviceType, event);
...
function smartthings2homebridge(aid, iid, value) {
  var body = {
    "characteristics": [{
      "aid": aid,
      "iid": iid,
  ...
  homebridge.HAPcontrolByDeviceID(homebridge_instance.deviceID, JSON.stringify(body), function(err, status) {
    console.log('send smartthings command to the accessory: ', JSON.stringify(body), err, status);
  }.bind(this));
}
