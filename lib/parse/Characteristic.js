var debug = require('debug')('smartthings:Characteristic');
...
function Characteristic(devices, context) {
  ...
  var actions = _getActions(this.description, context, devices);
  this.cookie = actions.cookie;
  this.reportState = actions.reportState;
  // debug("lookup", this.description, context.service); // Programmable Switch Event Doorbell
  // Don't lookup capabilities for PSE unless it is a Doorbell
  if (this.description === "Programmable Switch Event" && context.service === "Doorbell") {
    this.capabilities = messages.lookupCapabilities(this.description, context.options);
  } else if (this.description !== "Programmable Switch Event") {
    this.capabilities = messages.lookupCapabilities(this.description, context.options, undefined, devices);
  }
...
function _getActions(description, context, devices) {
  var cookie = {};
  var reportState = [];
  var hapEvents = [];
  // debug("_getActions", description, devices);
  switch (description) {
    case "Active": // Active on a Fan 2 aka Dyson or Valve
    case "On": // Accessory On/Off
      // debug("On/Active", context.name);
      reportState.push(messages.reportState("SmartThings.PowerController", context));
      cookie["TurnOn"] = messages.cookieV(1, context);
      cookie["TurnOff"] = messages.cookieV(0, context);
      break;
    case "Rotation Speed": // RotationSpeed
      reportState.push(messages.reportState("SmartThings.PowerLevelController", context));
      cookie["AdjustPowerLevel"] = messages.cookie(context);
      cookie["SetPowerLevel"] = messages.cookie(context);
      break;
      /*
    case "Configured Name": // Channel Change Hack
      cookie["Channel"] = messages.cookie(context);
      debug("Channel", context);
      break;
      */
    case "Active Identifier": // Channel Change Hack
      cookie["Active Identifier"] = messages.cookie(context);
      break;
    ...
    case "Current Temperature": // Current Temperature
      reportState.push(messages.reportState("SmartThings.TemperatureSensor", context));
      break;
    case "Motion Detected": // Motion sensor state
      reportState.push(messages.reportState("SmartThings.MotionSensor", context));
    ...
    case "Current Position": // Current Door state
      reportState.push(messages.reportState("SmartThings.ContactSensor", context));
      ...
      hapEvents[JSON.stringify({
        ...
        "endpointID": context.endpointID,
        "friendlyName": context.name,
        "0": "DETECTED", // Aka open for SmartThings
        "1": "NOT_DETECTED",
        //  "2": "DETECTED",
        //  "3": "DETECTED",
        //  "4": "DETECTED",
    ...
    case "Target Door State": // TargetDoorState
      if (context.options.door) {
        cookie["SetMode"] = messages.cookie(context); // Open
        reportState.push(messages.reportState("SmartThings.ModeController", context));
        // reportState.push(messages.reportState("SmartThings.RangeController", context));
        // cookie["SetRangeValue"] = messages.cookie(context);
        // cookie["AdjustRangeValue"] = messages.cookie(context);
      } else {
        reportState.push(messages.reportState("SmartThings.PowerController", context));
    ...
    case "Occupancy Detected": // Occupany Sensor
      reportState.push(messages.reportState("SmartThings.ContactSensor", context));
      ...
      // I think that SmartThings and Apple are backwards
      ...
      hapEvents[JSON.stringify({
        ...
        "endpointID": context.endpointID,
        "friendlyName": context.name,
        "1": "DETECTED", // Aka open for SmartThings
        "0": "NOT_DETECTED",
        "template": "ContactSensor"
      };
      break;
    case "Programmable Switch Event": // aka Doorbell
      ...
      if (context.service === 'Doorbell') { // Only for PSE's that are part of the Doorbell service
        hapEvents[JSON.stringify({
          "deviceID": context.deviceID,
    ...
    case "Color Temperature": // Color Temperature
      reportState.push(messages.reportState("SmartThings.ColorTemperatureController", context));
    ...
    case "Target Position": // WindowCoverings Target Position
      ...
      if (context.options.blind) {
        ...
        reportState.push(messages.reportState("SmartThings.PowerLevelController", context));
        if (devices.maxValue) {
          cookie["TurnOn"] = messages.cookieV(devices.maxValue, context);
        } else {
          cookie["TurnOn"] = messages.cookieV(100, context);
        }
        if (devices.minValue) {
          cookie["TurnOff"] = messages.cookieV(devices.minValue, context);
        } else {
          cookie["TurnOff"] = messages.cookieV(0, context);
        }
        cookie["AdjustPowerLevel"] = messages.cookie(context);
    ...
    case "Volume": // Speaker volume
      ...
      reportState.push(messages.reportState("SmartThings.Speaker", context));
    ...
    case "Preset": // Bose Sound link presets
      // case "91288267": // This is in the homebridge-bose-soundtouch plugin
      reportState.push(messages.reportState("SmartThings.ChannelController", context));
      for (var i = devices.minValue; i <= devices.maxValue; i = i + devices.minStep) {
        cookie[i] = messages.cookieV(i, context);
      }
      break;
    case "Volume Selector": // Speaker volume
      // case "91288267": // This is in the homebridge-bose-soundtouch plugin
      reportState.push(messages.reportState("SmartThings.StepSpeaker", context));
      cookie["AdjustVolume"] = messages.cookie(context);
      break;
      /*
    ...
    case "Target Temperature": // Target Temperature
      reportState.push(messages.reportState("SmartThings.ThermostatControllertargetSetpoint", context));
    ...
    case "Heating Threshold Temperature": // Target Temperature
      // reportState.push(messages.reportState("SmartThings.ThermostatController", context));
      reportState.push(messages.reportState("SmartThings.ThermostatControllerlowerSetpoint", context));
      cookie["lowerSetpoint"] = messages.cookie(context);
      break;
    case "Cooling Threshold Temperature": // Target Temperature
      // reportState.push(messages.reportState("SmartThings.ThermostatController", context));
      reportState.push(messages.reportState("SmartThings.ThermostatControllerupperSetpoint", context));
    ...
    case "Target Heating Cooling State":
      reportState.push(messages.reportState("SmartThings.ThermostatControllerthermostatMode", context));
      // reportState.push(messages.reportState("SmartThings.ThermostatController", context));
    ...
    case "Current Heating Cooling State":
      // SmartThings doesn't report Furnace state.  Issue # 312
      // reportState.push(messages.reportState("SmartThings.ThermostatControllerthermostatMode", context));
    ...
    case "Lock Current State":
      reportState.push(messages.reportState("SmartThings.LockController", context));
      break;
