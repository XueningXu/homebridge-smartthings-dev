       ...
       if (verifyDeviceInList(deviceList, accessory.info.Name)) {
          ...
          for (var index in accessory.services) {
            ...
            for (var index in service.characteristics) {
              // check if this characteristics has write permission
              if (service.characteristics[index].perms.includes('pw')) {
              ...
              } else {   // if not write permission, which means this characteristics cannot be changed
                continue;
              }
            }
          }
